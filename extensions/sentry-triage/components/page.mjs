    ...
    <script>
      ...
      document.addEventListener("change", (e) => {
        ...
        if (orgSel && orgSel.tagName === "SELECT") {
          // A new org invalidates the old project. Reset to all projects and
          // re-scan; also refresh the autocomplete list for the new org (instant
          // from cache, else a loading hint) so it's ready by the time the scan
          // lands.
          currentProject = "";
