...
- `.github/dbmigration/Reports/*` — testing plan, migration findings/results, bug reports
- `.github/dbmigration/DDL/Oracle/*` — Oracle stored procedure, function, table, and view definitions (pre-migration)
- `.github/dbmigration/DDL/Postgres/*` — PostgreSQL stored procedure, function, table, and view definitions (post-migration)

## Orchestration entrypoint

- Use the router at [.github/agents/dbmigration-expert/router.prompt.md](.github/agents/dbmigration-expert/router.prompt.md) as the primary entrypoint. It interprets the user goal, checks prerequisites, and hands off to the task-specific prompts below in the correct order.
...
- Provide Oracle and Postgres DDL scripts under `{SOLUTION_ROOT}/.github/dbmigration/DDL/` so the router and subagents have necessary context.
- If you want the router to create missing DDL folders or verify extensions, state that when invoking it.
- The `dbmigration` skill (under `.github/skills/dbmigration/`) provides validation checklists and reference insights for Oracle→Postgres migration patterns.
