import { AwesomeCopilotServer } from '../src/index';
...
describe('AwesomeCopilotServer Integration', () => {
  let server: AwesomeCopilotServer;

  beforeEach(() => {
    server = new AwesomeCopilotServer();
  });

  describe('server initialization', () => {
    it('should create server instance', () => {
      expect(server).toBeInstanceOf(AwesomeCopilotServer);
    });
  });
  ...
  describe('tool listing', () => {
    it('should list all available tools', async () => {
      const mockRequest = {};
      const handler = (server as any).server.setRequestHandler.mock.calls.find(
    ...
    it('should have correct tool schemas', async () => {
      ...
      const searchAllTool = result.tools.find((tool: any) => tool.name === 'search_all');
      ...
      const loadAgentTool = result.tools.find((tool: any) => tool.name === 'load_agent');
  ...
  describe('tool calling', () => {
    ...
    beforeEach(() => {
      callToolHandler = (server as any).server.setRequestHandler.mock.calls.find(
    ...
    it('should handle search_all tool call', async () => {
      ...
      const searchTools = (server as any).searchTools;
    ...
    it('should handle load_agent tool call', async () => {
      ...
      // Mock the load tools
      const loadTools = (server as any).loadTools;
