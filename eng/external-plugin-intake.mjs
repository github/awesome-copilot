...
export async function evaluateExternalPluginIssue({ issue, token, runId, owner, repo } = {}) {
  ...
  const commentBody = valid
    ? [
        ...
        parsed.plugin.source.ref ? `- **Ref:** [\`${parsed.plugin.source.ref}\`](https://github.com/${parsed.plugin.repository}/tree/${parsed.plugin.source.ref})` : undefined,
        parsed.plugin.source.sha ? `- **SHA:** [\`${parsed.plugin.source.sha}\`](https://github.com/${parsed.plugin.repository}/tree/${parsed.plugin.source.sha})` : undefined,
