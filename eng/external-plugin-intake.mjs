...
function buildMergedIntakeComment(baseResult, qualityResult, runId, owner, repo) {
  ...
  return [
    ...
    baseResult.plugin?.source?.sha ? `- **SHA:** [${baseResult.plugin.source.sha}](https://github.com/${encodeRepoPath(baseResult.plugin.source.repo)}/tree/${encodeURIComponent(baseResult.plugin.source.sha).replaceAll("%2F", "/")})` : undefined,
    "",
    qualitySection,
    "",
    "",
