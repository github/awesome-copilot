...
function buildMergedIntakeComment(baseResult, qualityResult, runId, owner, repo) {
  ...
  return [
    ...
    baseResult.plugin?.source?.ref ? `- **Ref:** [\`${baseResult.plugin.source.ref}\`](https://github.com/${encodeRepoPath(baseResult.plugin.source.repo)}/tree/${encodeURIComponent(baseResult.plugin.source.ref)})` : undefined,
    baseResult.plugin?.source?.sha ? `- **SHA:** [\`${baseResult.plugin.source.sha}\`](https://github.com/${encodeRepoPath(baseResult.plugin.source.repo)}/tree/${encodeURIComponent(baseResult.plugin.source.sha)})` : undefined,
    "",
    qualitySection,
    "",
    "",
...
export async function evaluateExternalPluginIssue({ issue, token, runId, owner, repo } = {}) {
  ...
  const commentBody = valid
    ? [
        ...
        parsed.plugin.source.ref ? `- **Ref:** [\`${parsed.plugin.source.ref}\`](https://github.com/${encodeRepoPath(parsed.plugin.source.repo)}/tree/${encodeURIComponent(parsed.plugin.source.ref).replaceAll("%2F", "/")})` : undefined,
        parsed.plugin.source.sha ? `- **SHA:** [\`${parsed.plugin.source.sha}\`](https://github.com/${encodeRepoPath(parsed.plugin.source.repo)}/tree/${encodeURIComponent(parsed.plugin.source.sha).replaceAll("%2F", "/")})` : undefined,
