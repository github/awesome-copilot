...
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP OAuth sign-in now supports CIMD, letting an MCP server resolve OAuth client metadata from a hosted document URL instead of requiring manual client registration—useful for enterprise identity providers that publish client metadata centrally.
