...
> **Client ID Metadata Document support (v1.0.83+)**: MCP OAuth sign-in now supports the Client ID Metadata Document (CIMD) approach, an alternative to Dynamic Client Registration where the client's identity and metadata are published at a URL instead of registered per-server. This broadens compatibility with MCP servers that support CIMD-based OAuth flows.
