...
**include-custom-instructions** *(v1.0.86+, Copilot CLI)*: Set to `true` to have the agent also load repository instruction files — `AGENTS.md`, `copilot-instructions.md`, and `CLAUDE.md` — in addition to its own persona instructions. This is useful when you want a specialized agent to still follow your team's shared conventions rather than operating in isolation:

```yaml
---
name: 'Release Manager'
description: 'Automates release preparation, changelog generation, and version bumping'
model: Claude Sonnet 4
