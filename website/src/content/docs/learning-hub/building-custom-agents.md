...
**Model fallback lists** *(v1.0.83+)*: `model` can also be an array of models, tried in order until one is available to you—useful when an agent's preferred model is temporarily rate-limited or not enabled for your account:
...
Add `model-policy: required` to keep any manual model changes restricted to that list—so a user can't switch the agent to a model outside the approved set mid-session.
