...
*(v1.0.85+)* Opt in to context management tools for agents and subagents from `/settings`, giving agents more direct control over how their context window is curated during long-running sessions.

### Concise Transcript View

*(v1.0.85+)* Set `transcriptView` to `"concise"` to group tool activity into expandable work summaries instead of a flat scrolling log — useful for keeping long sessions readable while still letting you drill into the details of any step.

### Repository Instructions in Custom Agents

*(Copilot CLI v1.0.86+)* Custom agents can opt into repository instruction files by setting `include-custom-instructions: true` in their frontmatter, layering `AGENTS.md`, `copilot-instructions.md`, and `CLAUDE.md` on top of the agent's own persona and instructions. See [Building Custom Agents](../building-custom-agents/) for the frontmatter syntax.
