...
A: Skills can be invoked in several ways. Users can type the skill name as a `/command` (e.g., `/generate-tests`). As of v1.0.44, **multiple skills can be invoked in a single message** — for example, `/generate-tests /explain-architecture` invokes both in sequence. Slash commands can also appear mid-input, not just at the start of a message. Agents can also discover and invoke skills automatically based on the skill's description and the user's intent.
