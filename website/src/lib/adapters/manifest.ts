...
const SECTION_CONFIG: Record<
  string,
  { icon: string; accent: Accent; href: string }
> = {
...
export function adaptManifest(manifest: ManifestItem): ManifestSummary {
  // Spread into a plain Record so we can index it by string key without TypeScript
  // complaining that a specific-key interface is not indexable by arbitrary strings.
