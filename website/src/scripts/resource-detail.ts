...
const FEATURE_ICON_PATHS: Record<string, string> = {
  ...
  workflows:
    "M5 4h6v6H9v2h6v-2h-2V4h6v6h-2v4H9v2h2v4H5v-6h2v-4H5V4Zm2 2v2h2V6H7Zm8 0v2h2V6h-2Zm-8 10v2h2v-2H7Z",
  plugins:
    "M8 3h3v4h2V3h3v4h2v5h-3v3h3v3h-3v3h-3v-3H9v3H6v-3H3v-3h3v-3H3V7h5V3Zm1 7v5h6v-5H9Z",
  tools:
    "M4 5h16v10H4V5Zm2 2v6h12V7H6Zm2 11h8v2H8v-2Zm1-9h2v2H9V9Zm3 0h4v2h-4V9Z",
  share:
    "M18 8h3v3h-3v2h-2v2h-2v2h-2v2h-2v2H8v-2H6v-2H4v-2H2v-3h3v2h2v2h2v2h2v-2h2v-2h2v-2h2V8Zm-4-4h4v4h-4V4Z",
...
function renderFeatureCards(features: DetailData["features"]): void {
  ...
  for (const feature of features) {
    ...
    path.setAttribute(
      "d",
      FEATURE_ICON_PATHS[feature.icon] ?? FEATURE_ICON_PATHS.info,
    );
    icon.append(path);
...
function adaptSkill(item: SkillItem): DetailData {
  ...
  return {
    ...
    tags: [
      item.category,
      item.hasAssets ? "assets" : "no-assets",
    ...
    features: [
      ...
      {
        title: "Assets",
        value: item.hasAssets
          ? `${item.assetCount} bundled asset${item.assetCount === 1 ? "" : "s"}`
          : "(none)",
        icon: "archive",
      },
      {
        title: "Files",
        value: `${item.files.length} file${item.files.length === 1 ? "" : "s"}`,
        icon: "document",
      },
    ],
    featureTags: [],
...
function adaptHook(item: HookItem): DetailData {
  ...
  return {
    ...
    label:
      item.hooks.length === 1 ? item.hooks[0] : `${item.hooks.length} events`,
    accent: stableAccent(item.id),
    ...
    features: [
      ...
      { title: "Tags", value: joinOrNone(item.tags), icon: "filter" },
      {
        ...
        value: item.assets.length
          ? `${item.assets.length} bundled asset${item.assets.length === 1 ? "" : "s"}`
          : "(none)",
        icon: "archive",
      },
    ],
...
function adaptWorkflow(item: WorkflowItem): DetailData {
  return {
    ...
    features: [
      {
        title: "Triggers",
        value: joinOrNone(item.triggers),
        icon: "workflows",
      },
      { title: "Source", value: item.path, icon: "document" },
      {
        title: "Updated",
        value: item.lastUpdated || "(unknown)",
        icon: "info",
      },
    ],
    featureTags: [],
...
function adaptPlugin(item: PluginItem): DetailData {
  const repository =
    item.repository ||
    item.homepage ||
    `https://github.com/${REPO_IDENTIFIER}/tree/main/${item.path}`;
  return {
    id: item.id,
    title: item.name,
    description: item.description,
    ...
    installCommand: item.external
      ? `copilot plugin install ${item.name}`
      : `copilot plugin install ${item.name}@awesome-copilot`,
    ...
    features: [
      {
        title: "Resources",
        value: `${item.itemCount} bundled resource${item.itemCount === 1 ? "" : "s"}`,
        icon: "plugins",
      },
      {
        title: "Source",
        value: item.external ? "external" : "awesome-copilot",
        icon: item.external ? "open" : "github",
      },
...
function adaptTool(item: ToolItem): DetailData {
  ...
  return {
    ...
    installCommand:
      item.configuration?.content?.trim() ||
    ...
    features: [
      ...
      {
        title: "Requirements",
        value: joinOrNone(item.requirements),
        icon: "terminal",
      },
      {
        title: "Features",
        value: formatCount(item.features.length, "capability"),
        icon: "bolt",
      },
    ],
...
function renderDetail(detail: DetailData, kind: ResourceKind): void {
  ...
  document
    ...
    ?.setAttribute("content", detail.description);
  ...
  for (const code of document.querySelectorAll<HTMLElement>(
    ...
    code.textContent = detail.installCommand;
...
function initDetailActions(detail: DetailData, kind: ResourceKind): void {
  ...
  const heroInstallBtn = document.getElementById(
    "hero-install-btn",
  ) as HTMLButtonElement | null;
  const dialog = document.getElementById(
    "install-modal",
  ) as HTMLDialogElement | null;
  const vscodeLink = document.getElementById(
    "install-modal-vscode",
  ) as HTMLAnchorElement | null;
  const insidersLink = document.getElementById(
    "install-modal-insiders",
  ) as HTMLAnchorElement | null;
  ...
  if (downloadBtn) {
    downloadBtn.addEventListener("click", async () => {
      if (detail.zipFiles && detail.zipFiles.length > 0) {
        ...
        showToast(
          ok ? "Download started!" : "Download failed",
          ok ? "success" : "error",
        );
...
export async function initResourceDetail(kind: ResourceKind): Promise<void> {
  ...
  if (!rawId) {
    renderError(
      kind,
      `Missing ${kind} id. Go back to the listing and choose a resource.`,
    );
    return;
  }
  ...
  try {
    const response = await fetch(
      `${import.meta.env.BASE_URL}${DATA_URLS[kind]}`,
    );
    if (!response.ok)
      throw new Error(`Failed to load ${kind} data (${response.status})`);
