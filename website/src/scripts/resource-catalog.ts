...
async function loadItems<TRaw>(
  dataUrl: string,
  toClientItem: (item: TRaw) => CatalogClientItem,
): Promise<CatalogClientItem[]> {
    ...
    if (config.type === "flag") {
      ...
      state[config.param] =
        config.type === "multi"
          ...
          : [getQueryParam(config.param) || config.defaultValue || ""].filter(
              Boolean,
            );
    }
  }
...
function sortItems(
  ...
  return [...items].sort((a, b) => {
    ...
    if (sortBy === "featured") {
      const featured =
        Number((b.filters.featured ?? []).includes("true")) -
        Number((a.filters.featured ?? []).includes("true"));
...
function renderCard(item: CatalogClientItem): string {
  ...
  const tags = item.tags
    .slice(0, 3)
    .map(
      (tag) => `<span class="badge badge--neutral">${escapeHtml(tag)}</span>`,
    )
    .join("");
...
function updateFilterDisplays(
  ...
  for (const config of configs) {
    const display = document.querySelector<HTMLElement>(
      `.filter-display[data-param="${config.param}"]`,
    );
    ...
    display.textContent =
      active.length > 0
        ? `${config.label.toLowerCase()}: ${active.join(", ")}`
        : `${config.label.toLowerCase()}: all`;
  }
}
...
function wireFilterSelects(
  ...
  document
    .querySelectorAll<HTMLSelectElement>("select[data-filter-param]")
    .forEach((select) => {
      const config = configs.find(
        (item) => item.param === select.dataset.filterParam,
      );
      if (!config) return;
      ...
      const values =
        config.type === "multi"
          ? getQueryParamValues(config.param)
          : [getQueryParam(config.param) || config.defaultValue || ""];
      ...
      select.addEventListener("change", () => {
        if (config.type === "multi") {
          updateQueryParams({
            setAll: {
              [config.param]: Array.from(select.selectedOptions)
          ...
          const value =
            select.value && select.value !== config.defaultValue
              ? select.value
              : null;
          updateQueryParams({ set: { [config.param]: value } });
        }
...
function wireFilterToggles(
  ...
  document
    .querySelectorAll<HTMLButtonElement>("button[data-filter-param]")
    .forEach((button) => {
      const config = configs.find(
        (item) => item.param === button.dataset.filterParam,
      );
...
export async function initResourceCatalog<TRaw>(
  config: CatalogConfig<TRaw>,
): Promise<{ applyAndRender: () => void }> {
  ...
  const loadMore = document.querySelector<HTMLButtonElement>(
    config.loadMoreSelector,
  );
  const loadSentinel = document.querySelector<HTMLElement>(
    config.loadSentinelSelector,
  );
  const clearButton = document.querySelector<HTMLButtonElement>(
    config.clearSelector,
  );
  const emptyClear = document.querySelector<HTMLButtonElement>(
    config.emptyClearSelector,
  );
  ...
  try {
    ...
    console.error(error);
    if (countEl)
      countEl.textContent = `Failed to load ${config.resourceNamePlural}`;
  ...
  function renderProgress(): void {
    ...
    if (grid)
      grid.innerHTML = currentItems
        ...
        .join("");
    if (countEl)
      countEl.textContent = `Showing ${visibleItems} of ${currentItems.length} matching ${config.resourceName}${currentItems.length === 1 ? "" : "s"} (${items.length} total)`;
    if (loadMore) {
  ...
  function clearFilters(): void {
    ...
    for (const filterConfig of config.filterConfigs)
      remove[filterConfig.param] = null;
    ...
    document
      ...
      .forEach((select) => {
        for (const option of select.options)
          option.selected =
            option.value === "" || option.value === select.dataset.defaultValue;
      });
    applyAndRender();
  }
  ...
  initSearch<CatalogClientItem>(
    ...
    },
    {
      ...
      statusText(query, resultCount, totalCount) {
        if (!query) return "";
        if (resultCount === 0)
          return `No ${config.resourceNamePlural} match "${query}"`;
