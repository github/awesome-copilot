...
| Orchestrator cannot verify `closingIssuesReferences` | The PR body lacks the marked `Refs #N` line, targets a branch other than `main`, or another open PR already closes the issue | Restore `Refs #N` followed by `<!-- fork-issue-link: #N -->`, ensure this is the canonical implementation PR into `main`, then re-run the orchestrator |
