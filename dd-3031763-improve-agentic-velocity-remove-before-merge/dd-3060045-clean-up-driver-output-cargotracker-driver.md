# Clean up the Cargo Tracker shepherd-task control driver output
...
3. which normal workflow stages are replaced by deterministic fixture preparation;
4. the exact PowerShell invocation of each canonical shepherd-task script used by the experiment;
5. the current campaign and per-issue progress;
6. the durable evidence produced by the run.

Provide opt-in switches for exposing intermediate child-process output. Preserve all current campaign behavior, side effects, validation, failure semantics, redaction, artifacts, and cleanup policy.
...
Do not change the Cargo Tracker fixture scripts unless a pre-existing non-destructive contract test must be updated to assert the driver behavior. Prefer changing only the driver and the smallest directly related contract test.

Do not apply this output design to the simple-math driver, treatment/control driver, Bash scripts, or other experiment drivers in this change.
...
The driver does not invoke stage 10 or perform the research gate live. Its fixture writes a deterministic, already-resolved ignorance-reduction plan. The default output must state this substitution clearly without implying that normal shepherd-task users may omit planning or research.

Stage 25 invokes stages 30 and 40 serially for each issue and invokes stage 50 after success or failure. The output must not mislabel stage 50 as stage 40.
...
Show the complete inherited output of fixture scripts directly invoked by the driver:
...
Show output from native commands invoked directly by the driver, including relevant `git` and `gh` setup and synchronization operations.
...
At minimum, review these direct operations:
...
Also review every other direct `git` and `gh` invocation in the driver and classify it deliberately as:
...
After resolving the repository and paths, print a compact run header that includes the existing repository, workarea, checkout, worktree, source branch, baseline branch, and control branch information.
...
Do not describe baseline publication, contract checks, repository cloning, or final experiment verification as shepherd-task lifecycle stages.
...
- locate the unique campaign directory as today;
...
- this control fixture has written an already-resolved plan instead;
...
Then state that stage 15 generated a launcher which invoked the stage-20 skill and that the fixture verified the resulting issue bodies. Do not print the generated launcher's internal output by default.
...
& "<installed shepherd-task-25-given-list.ps1 path>" `
    -TaskIssues "<actual comma-separated issue list>" `
    -CampaignMetadataDirectory "<actual directory>"
```
...
- process the supplied issue list serially;
...
- proceed to the next issue only after the current issue is merged;
...
When `-ShowShepherdTaskScriptOutput` is not selected, the driver must still communicate meaningful progress during the long-running stage-25 operation. Do not change core scripts to achieve this. Choose a driver-only approach that does not alter campaign semantics, for example:

- print a clear message before handing control to stage 25, including all issue numbers and the evidence directory that will contain the run; and
...
Continue performing preflight, contracts, clone, baseline publication, worktree creation, synchronization, fixture verification, and evidence collection.

Present these as concise driver-owned statuses, for example:

```text
[shepherd-control] Experiment setup: validating prerequisites
[shepherd-control] Experiment setup: publishing immutable baseline
...
- the deterministic substitution for stage 10 and research;
...
Do not run the paid end-to-end control campaign as validation.
...
10. The canonical script names and required argument names appear in the driver:
    ...
    - `shepherd-task-25-given-list.ps1`
    - `-TaskIssues`
11. The driver still invokes the fixture wrappers needed by the experiment.
12. Hidden successful child output and automatically revealed failing child output are covered through a safe stub script or static/isolated helper test.
13. Enabled child output is passed through.
14. Nonzero child exit codes remain failures.
15. The invocation display helper quotes paths and values containing spaces.
16. No core shepherd-task script or skill is modified.
...
```powershell
pwsh -NoLogo -NoProfile -File .\plugins\shepherd-task\test\cargotracker-add-change-arrival-deadline-feature\07-driver-encoding-contract.ps1
```
...
- the operational change is confined to the requested driver;
- only the smallest directly related contract test is additionally changed or added;
