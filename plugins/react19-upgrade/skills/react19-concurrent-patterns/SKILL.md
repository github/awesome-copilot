...
description: Provides patterns for preserving and correctly using React 19's new concurrent features  useTransition, useDeferredValue, Suspense for data fetching, use(), useOptimistic, and Actions  during and after a React 19 migration. Use this skill whenever a React 19 migrated codebase needs to adopt new React 19 APIs, when deciding whether to refactor loading states to use Suspense, when adding optimistic updates, or when an agent needs to verify existing Suspense/concurrent patterns are React 19 compatible. Also use this skill to ensure that existing concurrent mode patterns from React 18 are not accidentally broken during the React 19 migration.
---
