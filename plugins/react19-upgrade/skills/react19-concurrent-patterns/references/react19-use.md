...
The `use()` hook is React 19's answer for "unwrapping" promises and context without being inside a component. It enables a cleaner pattern for async data and avoids the `<Suspense>` wrapper complexity that sometimes required separate components.
...
```jsx
// React 18  required Suspense boundary + separate component:
const UserComponent = React.lazy(async () => {
