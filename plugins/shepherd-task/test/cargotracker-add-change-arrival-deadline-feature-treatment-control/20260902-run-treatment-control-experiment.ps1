...
.DESCRIPTION
    Uses the installed shepherd-task plugin, verifies and publishes the exact
    prepared Cargo Tracker baseline, runs five-task campaign lesson propagation
    treatment and control arms serially, verifies both completed campaigns, and
...
function Get-CampaignHandoff {
    ...
    if ($issueNumbers.Count -ne 5 -or
        @($issueNumbers | Where-Object { $_ -notmatch '^[1-9][0-9]*$' }).Count -ne 0) {
        throw "Experiment handoff must contain exactly five positive issue numbers: $($handoffFiles[0].FullName)"
    }

    return [pscustomobject]@{
        Path = $handoffFiles[0].FullName
        Data = $handoff
        IssueNumbers = @($issueNumbers | ForEach-Object { [int]$_ })
...
try {
    ...
    $installedBaselineFixture = Join-Path $FixtureRoot '00-prepare-test-baseline.ps1'
    if (-not (Test-Path -LiteralPath $installedBaselineFixture -PathType Leaf)) {
        throw "Installed Cargo Tracker fixture was not found at '$FixtureRoot'. Install shepherd-task before running this driver."
    }
    ...
    Write-ExperimentStatus 'Running offline lesson-default, stage-15, stage-40, session-outcome, recovery-driver, and encoding contract checks...'
    ...
    Invoke-CheckedPwshScript -Path (Join-Path $FixtureRoot '12-session-outcome-contract.ps1')
    Invoke-CheckedPwshScript -Path (Join-Path $FixtureRoot '13-resume-driver-contract.ps1')
    ...
    Write-ExperimentStatus 'Verifying and publishing the prepared Cargo Tracker shared baseline...'
    Set-Location -LiteralPath $Target
    ...
    foreach ($issueNumber in @($TreatmentHandoff.IssueNumbers | Select-Object -Skip 1)) {
        $sessionPattern = "phase1-task-*-$issueNumber.md"
        $sessions = @(
            Get-ChildItem -LiteralPath $TreatmentRun.FullName -File `
                -Filter $sessionPattern
        )
...
4. Compare each of the five paired tasks separately.
   State explicitly that the first treatment task cannot benefit from prior
   campaign lessons. Treat tasks 2 through 5 as successive
   forward-propagation observations.
5. Determine whether treatment lessons were captured, validated, merged,
   delivered to each later issue, and observably reflected in implementation
   or tests. Distinguish proven delivery from inferred cognitive use.
...
"@
    ...
    $summary = [ordered]@{
        ...
        treatment = [ordered]@{
            ...
            postFirstIssueStage30Sessions = @($treatmentPostFirstIssueSessions.FullName)
        }
        control = [ordered]@{
    ...
    Write-Host 'Treatment post-first stage 30 sessions:'
    $treatmentPostFirstIssueSessions | ForEach-Object {
    ...
    Write-Host 'Semantic lesson use still requires inspection of the reported treatment post-first transcripts and PR changes.' -ForegroundColor Yellow
}
catch {
    Write-Host ''
