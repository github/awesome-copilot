...
<#
.SYNOPSIS
    Runs the Cargo Tracker shepherd-task control campaign end to end.

.DESCRIPTION
    Uses the installed shepherd-task plugin, verifies and publishes the exact
    prepared Cargo Tracker baseline, runs one five-task campaign with lesson
    propagation disabled, verifies the completed campaign, and preserves all
    evidence.
...
function Get-CampaignHandoff {
    ...
    if ($issueNumbers.Count -ne 5 -or
        ...
        throw "Control handoff must contain exactly five positive issue numbers: $($handoffFiles[0].FullName)"
    }
...
try {
    ...
    Write-Host '=== shepherd-task Cargo Tracker run ===' -ForegroundColor Cyan
    ...
    foreach ($contract in @(
        ...
        '10-cargotracker-fixture-contract.ps1',
    ...
    if ($ValidateInstalledOnly) {
        Write-ControlStatus `
            'Installed Cargo Tracker driver validation completed without paid or mutating operations.' `
            -Color Green
        return
    }
    ...
    if ($BaselineSha -ne $ExpectedBaselineSha) {
        throw "Published baseline '$BaselineSha' does not match '$ExpectedBaselineSha'."
    }
