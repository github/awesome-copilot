<#
.SYNOPSIS
    Creates and verifies the five child issues for the control campaign.
#>
...
if ($experiment.expectedTaskCount -ne 5) { throw 'Experiment expectedTaskCount must equal 5.' }
...
if ($artifacts.TaskCount -ne 5) { throw "Stage 15 found $($artifacts.TaskCount) tasks; expected 5." }
...
if ($ledger.Count -ne 5) {
    $detail = Get-Stage20FailureDetail `
        -ArtifactDirectory $artifacts.ArtifactDirectory `
        -Repository $campaign.repository
    throw "Creation ledger contains $($ledger.Count) entries; expected 5. $detail"
}
