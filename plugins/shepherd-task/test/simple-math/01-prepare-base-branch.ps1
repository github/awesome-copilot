...
.DESCRIPTION
    ...
    real campaign issue, initializes stage 00 with its default lesson mode,
    writes a two-task plan and control metadata, then commits and pushes the
    campaign state.
#>
...
- Immutable baseline SHA: ``$BaselineSha``
...
if ([string]$campaign.lessonPropagation -ne 'off') {
    throw 'Initialized control campaign does not use lessonPropagation=off.'
}
...
git -C $repoRoot commit -m "test: initialize simple-math control campaign #$campaignIssueNumber"
