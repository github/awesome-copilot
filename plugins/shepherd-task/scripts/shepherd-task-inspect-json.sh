...
#
# Usage: ./shepherd-task-inspect-json.sh <jsonl-file> [count]
#   jsonl-file: path to the JSONL event log
#   count:     number of messages to show (default: 20)

set -euo pipefail

JSONL_FILE="${1:?Usage: $0 <jsonl-file> [count]}"
COUNT="${2:-20}"
...
jq -Rrc '
    ...
    | fromjson?
' "$JSONL_FILE" | tail -n "$COUNT" | while IFS= read -r line; do
