#!/usr/bin/env bash
#
# shepherd-task-interview-user-to-create-issues.sh — Interviews the user for all 12
...
REPO=$(read_required "1/12  REPO (OWNER/REPO format, e.g. github/copilot-sdk)")
...
PLAN_FILE_NAME=$(read_required "5/12  PLAN_FILE_NAME (name of the plan file within that directory)")

echo ""
echo "  Hint: copy the exact markdown heading from the plan."
QUESTIONS_SECTION=$(read_required "6/12  QUESTIONS_SECTION (exact heading of the resolved questions section)")
...
ISSUE_TYPE=$(read_required "10/12 ISSUE_TYPE (GitHub issue type for children)" "Task")

echo ""
echo "  Hint: repo-relative paths or constraints; comma-separated."
SUPPORTING_ARTIFACTS=$(read_required "11/12 SUPPORTING_ARTIFACTS (paths to spikes, screenshots, etc.)" "$PLAN_DIRECTORY")

UPDATE_PLAN_CHECKBOXES=$(read_required "12/12 UPDATE_PLAN_CHECKBOXES (true or false)" "false")
