...
class ExecutiveReportGenerator:
    ...
    def generate_executive_summary(
        ...
        system_prompt = """You are a senior AI and data analysis consultant.
Your task is to write professional, concise and compelling executive summaries.

CRITICAL RULES:
1. Tone: Professional, executive, with concrete data
2. Length: 2-3 paragraphs max (200-300 words)
3. Structure: Situation → Findings → Main recommendation
4. Data: Always include concrete numbers and metrics
5. Language: Avoid technical jargon. Use "documents" not "embeddings"
6. Impact: Highlight business value, not the technology
7. Format: Simple markdown, no headers"""
...
Main recommendation: {recommendations[0] if recommendations else 'Successful implementation'}

Write executive summary in {language}.
Include concrete numbers. Professional but accessible tone."""
    ...
    def generate_findings_section(
        ...
        system_prompt = """You are an AI documentation analyst.
Convert technical data into professional executive prose.
Keep formal but accessible tone."""

        findings_text = json.dumps(findings, indent=2, ensure_ascii=False)
        user_prompt = f"""Analyze these findings and write the "Findings" section in {language}:

{findings_text}

Format: 3-5 bullet points, each with an introductory phrase.
Include specific data. Highlight what is important."""
...
You create actionable and high-impact recommendations.
Tone: Professional, inspiring, results-oriented."""

        user_prompt = f"""Project context:
{context}

Generate 4-5 strategic recommendations in {language}:
- Each one must be actionable (not vague)
- Include timeline (short/medium/long term)
- Highlight business value

Format: bullet points with subtitle."""
    ...
    def generate_report(
        ...
        DocumentFormatting.add_subtitle(doc, f"Project: {metadata.project_name}")
