...
| hosted image missing `agent.py` (or other agent code) | build context too narrow — this shouldn't happen if `agent.py` lives inside `hosted/responses/` (the recommended layout); only occurs if you've moved shared agent code outside `hosted/` | keep `agent.py` next to `main.py` inside `hosted/responses/` so the default build context already includes it; if you deliberately keep shared code elsewhere, widen `hosted/azure.yaml`'s build context to reach it. |
