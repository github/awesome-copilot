...
description: 'Operate YYLO Ledger task management for repositories that use YYLO: create, list, search, get, mark, update, and archive tasks, manage blocked-by dependencies, ready and topologically ordered queues for parallel execution, multi-directory merges, and immutable cold-archive packs through the `yy ledger` or standalone `yylo-ledger` CLI. Use when the user mentions YYLO, YYLO Ledger, yy ledger, yylo-ledger, yy kanban, or Kanban tasks in a YYLO-based repository, or asks an agent to pick the next unblocked task or update task status. Do not use for Ledger wiki, workflow, or artifact Record operations, direct edits to Ledger storage, or non-YYLO Kanban boards.'
---
...
This skill drives the YYLO Ledger task CLI. In a full YYLO workspace the commands run through
`yy ledger` (install the orchestrator with `npm install --global '@yylo/cli@latest'`; Node.js
20.10 or newer, npm, and Git are required). In a standalone Ledger project the same task surface
is provided by the separately installed `yylo-ledger` CLI. Source and documentation:
<https://github.com/yylo-dev/yylo> · <https://github.com/yylo-dev/yylo-ledger> ·
