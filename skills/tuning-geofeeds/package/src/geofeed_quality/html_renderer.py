...
function renderBars(id,values,denominator){const root=byId(id);const total=Number(denominator);root.setAttribute('data-denominator',String(total));if(!total){root.append(node('div','empty','No observations in this analysis.'));return;}for(const [name,countValue] of Object.entries(values)){const count=Number(countValue);const row=node('div','bar-row');const nameNode=node('span','bar-label',label(name));const track=node('span','bar-track');track.setAttribute('role','img');track.setAttribute('aria-label',`${label(name)}: ${count} of ${total}`);const bar=document.createElementNS('http://www.w3.org/2000/svg','svg');bar.setAttribute('class','bar');bar.setAttribute('width',`${Math.min(100,(count/total)*100)}%`);bar.setAttribute('height','100%');bar.setAttribute('aria-hidden','true');track.append(bar);row.append(nameNode,track,node('span','bar-count',`${count} / ${total}`));root.append(row);}}
...
function renderGeoList(){const features=GEO.features;const root=byId('geo-list');byId('map-status').textContent=MAP.enabled?'Mapbox enhancement configured. The text list remains the accessible fallback.':'Offline map fallback active. Configure Mapbox explicitly to add an interactive map.';if(!features.length){root.append(node('div','empty','No validated MCP coordinates are present. No geometry was invented.'));return;}for(const feature of features.slice(0,100)){const item=node('div','geo-item');const geometry=feature.geometry;const description=geometry.type==='Point'?`Point · ${geometry.coordinates.join(', ')}`:`Bounding box · ${geometry.coordinates[0].slice(0,4).map(point=>point.join(', ')).join(' → ')}`;item.append(node('strong','',feature.properties.placeName||feature.properties.rowId));item.append(node('span','',`${feature.properties.rowId} · ${feature.properties.prefix} · ${description}`));root.append(item);}if(features.length>100)root.append(node('p','',`Showing 100 of ${features.length} geographic features. Download GeoJSON for all features.`));}
