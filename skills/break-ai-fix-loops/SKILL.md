...
description: 'Stop an AI coding agent from repeating ineffective fixes. Use when debugging cycles through similar patches, tests appear to pass while the real behavior is still wrong, the agent keeps retrying a command without gaining evidence, or a repair needs proof that its verifier can catch the defect and its rollback actually restores the prior state.'
license: MIT
metadata:
  source: https://github.com/twoicewoo/stop-ai-fix-loops
---
