...
| `Se necesita la aprobación del administrador` on device login | Used "Microsoft Graph Command Line Tools" (`14d82eec-...`) — not consented for non-admins in tenant | Ask IT to approve an app registration with least privilege, or use `Sites.Selected` (§32) |
